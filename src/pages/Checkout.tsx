...
const Checkout = () => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Redirect to Shopify's secure hosted checkout
          ...
          <div className="grid lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="card-premium p-8">
                <h2 className="font-display text-xl font-semibold mb-4">Shipping & Payment</h2>
                  ...
                  <>
                    ...
                    <div className="space-y-2 border-t border-border pt-4">
                      ...
                      <div className="flex justify-between font-body text-sm">
                        ...
                        <span className={shipping === 0 ? "text-primary font-medium" : ""}>{shipping === 0 ? "Free ✨" : `AED ${shipping}`}</span>
                      </div>
                    ...
                    >
                      {loading ? <><Loader2 size={16} className="animate-spin" /> Redirecting…</> : <><Lock size={16} /> Proceed to Checkout — AED {total.toFixed(0)}</>}
                    </button>
                  </>
                )}

                <div className="grid grid-cols-3 gap-2 pt-2">
                  {[{ icon: Lock, text: "Secure" }, { icon: Shield, text: "Protected" }, { icon: Truck, text: "Fast Delivery" }].map(({ icon: Icon, text }, i) => (
                    <div key={i} className="flex flex-col items-center gap-1 p-2 rounded-lg bg-secondary/50 text-center">
