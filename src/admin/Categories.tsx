...
import {
  getAllCategories, upsertCategory, deleteCategory as deletecat,
  fileToBase64, type Category,
...
export default function Categories() {
  ...
  async function toggleActive(cat: Category) {
    await upsertCategory({ id: cat.id, is_active: !cat.is_active });
    load();
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="font-body text-sm text-muted-foreground mt-0.5">{categories.length} categories · shared across all admins</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-card rounded-2xl border border-border">
          ...
          <p className="font-body text-sm text-muted-foreground">No categories yet.</p>
            ...
            <div key={cat.id} className={`bg-card rounded-2xl border overflow-hidden transition-all ${cat.is_active ? "border-border" : "border-border opacity-50"}`}>
              <div className="aspect-video bg-muted relative overflow-hidden">
                ...
                <div className="absolute top-2 right-2">
                  <button onClick={() => toggleActive(cat)} className={`w-6 h-6 rounded-full flex items-center justify-center transition-all ${cat.is_active ? "bg-green-500 text-white" : "bg-gray-400 text-white"}`}>
                    <Check size={12} />
                  </button>
                </div>
              </div>
              <div className="p-3 space-y-2">
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <button onClick={() => handleDelete(cat.id, cat.name)} className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg border border-red-200 text-red-500 font-body text-xs hover:bg-red-50 transition-all">
          ...
          <div className="bg-card rounded-2xl border border-border w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
                  ...
                  {form.image ? (
                    <>
                      ...
                      <div className="absolute inset-0 bg-black/30 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                        <Upload size={20} className="text-white" /><span className="text-white font-body text-xs ml-2">Change</span>
                      </div>
                    </>
                  ) : (
                    <div className="text-center">
                      {uploading
                        ...
                        : <><Upload size={24} className="text-muted-foreground mx-auto mb-1" /><p className="font-body text-xs text-muted-foreground">Click to upload image</p></>
                      }
                    </div>
                  )}
                </div>
              ...
              <div>
                ...
                <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value, slug: slugify(e.target.value) }))} placeholder="e.g. Premium Nuts"
              ...
              <div>
                ...
                <input value={form.slug} onChange={e => setForm(p => ({ ...p, slug: e.target.value }))} placeholder="e.g. premium-nuts"
